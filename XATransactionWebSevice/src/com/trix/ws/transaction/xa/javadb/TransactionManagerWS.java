...
public class TransactionManagerWS {
  ...
  public void addOperation(      
    ...
    System.out.println(gtrid + ' ' + bqual + ' ' + formatId + ' ' + startFlag + ' ' + values);
    try{
      ...
      String exceptionMessage =
        (e instanceof SQLException && startFlag == XAResource.TMNOFLAGS)?
            "Do nothing." : "Rollback.";      
      throw new StringException(exceptionMessage);
    }finally{
      if (xaConn != null){
        try{
          xaConn.close();         
  ...
  public String commit(String gtrid,String bqual,int formatId){
    System.out.println(gtrid + ' ' + bqual + ' ' + formatId);
    String outMessage;
    try{
      xaConn = xaDataSource.getXAConnection();
      xaRes = xaConn.getXAResource();
  ...
  public static void main(String[] args) {
    TransactionManagerWS TrMgr = new TransactionManagerWS();
    try{
    TrMgr.addOperation("GTRID", "BQUAL", 1, XAResource.TMNOFLAGS, "91, 'Burgas', 'Bulgaria', 'BOJ', 'Bulgarian', 'BG'");
    TrMgr.addOperation("GTRID", "BQUAL", 1, XAResource.TMJOIN, "92, 'Varna', 'Bulgaria', 'VAR', 'Bulgarian', 'BG'");
    TrMgr.addOperation("GTRID", "BQUAL", 1, XAResource.TMJOIN, "93, 'Sofia', 'Bulgaria', 'SOF', 'Bulgarian', 'BG'");
    TrMgr.commit("GTRID", "BQUAL", 1);
    System.err.println("Commited.");
    }catch(StringException se){
      se.printStackTrace();
      if (se.getMessage().equals("Rollback.")){
        TrMgr.rollback("GTRID", "BQUAL", 1);
